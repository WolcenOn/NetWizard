...
test('Capability UI expone dependencias, conflictos y límites',()=>{const m=UI.buildModel(project);const arp=m.devices[0].rows.find(r=>r.id==='arpInspection');const hsrp=m.devices[0].rows.find(r=>r.id==='hsrp');const lacp=m.devices[0].rows.find(r=>r.id==='lacp');assert.deepStrictEqual(arp.requires,['dhcpSnooping']);assert.ok(hsrp.conflictsWith.includes('vrrp'));assert.ok(lacp.limits.some(x=>x.includes('linkAggregations')));});
