...
incremental/commands/*.{set,cfg,conf,rsc}
incremental/rollback/*.{set,cfg,conf,rsc}
...
El registro incremental puede convertir un change set Junos `display set` en candidatos `set/delete`, un cambio Cisco IOS administrado en candidatos `.cfg`, un cambio FortiOS administrado en candidatos jerárquicos `.conf` y un cambio RouterOS v7 administrado en candidatos `.rsc`. Los demás fabricantes permanecen en revisión manual hasta disponer de un parser seguro. `deployment.requireExecutableIncremental: true` bloquea el paquete si cualquier cambio carece de adaptador ejecutable.
