...
(function initNetWizardDeploymentRunbook(root){
  ...
  function buildDeploymentPlan(project,options){
    ...
    const draft=arr(p.devices).map((device,index)=>{const override=obj(overrides[device.id]),phase=phaseFor(device,override),guide=guidanceFor(device),change=obj(changeByDevice.get(device.id));return{deviceId:device.id,deviceName:clean(device.name,100)||device.id,kind:deviceKind(device),vendor:clean(device.vendorOs,80)||'generic_network',phase:phase.id,phaseLabel:phase.label,phaseOrder:phase.order,requestedOrder:number(override.order,number(device.deploymentOrder,9999)),distance:number(distances.get(device.id),9999),dependsOn:deviceDependencies(p,device,phase,graph,distances,overrides),risk:riskFor(device,phase,p),serialGroups:serialGroups(p,device.id),estimatedMinutes:Math.max(5,number(override.estimatedMinutes,number(device.deploymentMinutes,15))),configPath:configPathFor(device,index,opts),change:{status:change.status||'baseline-required',applyMode:change.applyMode||'full-target',patchPath:change.patchPath||null,rollbackPatchPath:change.rollbackPatchPath||null,capturedAt:change.capturedAt||null,stats:obj(change.stats)},backup:guide.backup,validation:guide.validate,rollback:guide.rollback};});
    ...
    return{
      ...
      estimatedTotalMinutes:steps.reduce((total,step)=>total+step.estimatedMinutes,0)+Math.max(5,number(deployment.observationMinutes,15)),validationTargets,criticalServices,resilienceChecks,changeSet:{requestedMode:changeSet.requestedMode||'full',executionMode:changeSet.executionMode||'full-target',coverage:obj(changeSet.coverage)},
  ...
  function buildMarkdown(plan){
    ...
    let currentPhase='';for(const step of arr(p.steps)){if(step.phase!==currentPhase){currentPhase=step.phase;lines.push(`### ${step.phaseLabel}`,'');}const change=obj(step.change),stats=obj(change.stats),noChange=change.status==='no-change';const application=noChange?['No aplicar configuración: el objetivo normalizado coincide con el snapshot observado.','Ejecutar únicamente las validaciones y conservar evidencia.']:['Comparar el fichero objetivo y, si existe, el diff de revisión con la configuración activa.','Tratar el diff como evidencia; no pegarlo como comandos en el dispositivo.','Aplicar mediante el mecanismo transaccional/seguro del fabricante.','No continuar con el siguiente equipo hasta completar las validaciones.'];lines.push(`#### ${step.order}. ${step.deviceName} (${step.vendor})`,'',`- Riesgo: **${step.risk}**`,`- Configuración: \`${step.configPath}\``,`- Estado del cambio: **${change.status||'baseline-required'}** (${change.applyMode||'full-target'})`,`- Delta: +${stats.addedLines||0} / -${stats.removedLines||0} líneas`,`- Diff de revisión: ${change.patchPath?`\`${change.patchPath}\``:'no disponible'}`,`- Dependencias: ${step.dependsOn.length?step.dependsOn.join(', '):'ninguna'}`,`- Tiempo estimado: ${step.estimatedMinutes} minutos`,'','Backup previo:',markdownList(step.backup),'','Aplicación:',markdownList(application),'','Validación:',markdownList(step.validation),'','Rollback local:',noChange?'- [ ] No requiere rollback: no se aplica configuración.':markdownList(step.rollback),'');}
  ...
  function buildRollbackMarkdown(plan){const p=obj(plan),lines=[`# Checklist de rollback — ${p.projectName||'NetWizard'}`,'',`> ${p.snapshotWarning||''}`,'','## Preparación','- [ ] Identificar exactamente los dispositivos ya modificados.','- [ ] Confirmar el último backup real válido de cada uno.','- [ ] Notificar el rollback y congelar cambios adicionales.','','## Ejecución','',markdownList(p.rollbackPlan),'','## Orden inverso',''];for(const step of arr(p.steps).slice().reverse()){const change=obj(step.change),reverse=change.rollbackPatchPath;if(change.status==='no-change')lines.push(`- [ ] ${step.deviceName} — ${step.vendor} — sin rollback; paso de validación únicamente.`);else lines.push(`- [ ] ${step.deviceName} — ${step.vendor} — restaurar backup y ejecutar validaciones${reverse?` (diff inverso de apoyo: \`${reverse}\`)`:'.'}`);}lines.push('','## Cierre','- [ ] Confirmar recuperación de servicios y monitorización.','- [ ] Guardar logs, tiempos y causa del rollback.','- [ ] No reintentar hasta aprobar un plan corregido.','');return lines.join('\n');}
