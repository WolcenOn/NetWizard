...
(function initNetWizardDeploymentRunbook(root){
  ...
  function buildDeploymentPlan(project,options){
    ...
    const draft=arr(p.devices).map((device,index)=>{const override=obj(overrides[device.id]),phase=phaseFor(device,override),guide=guidanceFor(device);return{deviceId:device.id,deviceName:clean(device.name,100)||device.id,kind:deviceKind(device),vendor:clean(device.vendorOs,80)||'generic_network',phase:phase.id,phaseLabel:phase.label,phaseOrder:phase.order,requestedOrder:number(override.order,number(device.deploymentOrder,9999)),distance:number(distances.get(device.id),9999),dependsOn:deviceDependencies(p,device,phase,graph,distances,overrides),risk:riskFor(device,phase,p),serialGroups:serialGroups(p,device.id),estimatedMinutes:Math.max(5,number(override.estimatedMinutes,number(device.deploymentMinutes,15))),configPath:configPathFor(device,index,opts),backup:guide.backup,validation:guide.validate,rollback:guide.rollback};});
    ...
    return{
      ...
      estimatedTotalMinutes:steps.reduce((total,step)=>total+step.estimatedMinutes,0)+Math.max(5,number(deployment.observationMinutes,15)),validationTargets,criticalServices,resilienceChecks,
  ...
  function buildMarkdown(plan){
    ...
    let currentPhase='';for(const step of arr(p.steps)){if(step.phase!==currentPhase){currentPhase=step.phase;lines.push(`### ${step.phaseLabel}`,'');}lines.push(`#### ${step.order}. ${step.deviceName} (${step.vendor})`,'',`- Riesgo: **${step.risk}**`,`- Configuración: \`${step.configPath}\``,`- Dependencias: ${step.dependsOn.length?step.dependsOn.join(', '):'ninguna'}`,`- Tiempo estimado: ${step.estimatedMinutes} minutos`,'','Backup previo:',markdownList(step.backup),'','Aplicación:','- [ ] Comparar el fichero generado con la configuración activa.','- [ ] Aplicar mediante el mecanismo transaccional/seguro del fabricante.','- [ ] No continuar con el siguiente equipo hasta completar las validaciones.','','Validación:',markdownList(step.validation),'','Rollback local:',markdownList(step.rollback),'');}
