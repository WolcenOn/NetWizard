/* =========================================================
   ...
   Integra diagnósticos arquitectónicos y compatibilidad.
...
(function initNetWizardProductionGateArchitecture(root){
  ...
  function mergeIssues(){const seen=new Set(),out=[];for(const group of arguments)for(const issue of arr(group)){const key=[issue&&issue.code,issue&&issue.severity,issue&&issue.category,issue&&issue.deviceId,issue&&issue.capability,issue&&issue.message].join('\u0001');if(seen.has(key))continue;seen.add(key);out.push(issue);}return out;}
  function enhanceReport(project,options,baseReport){
    const gate=baseGate(),architecture=validator(),registry=capabilityRegistry();
    ...
    const issues=mergeIssues(report.issues,architectureReport.issues,compatibilityReport.issues);
    ...
    return Object.assign({},report,{ok:blocking.length===0,ready:status==='ready',canExport:blocking.length===0,status,issues,counts,architecture:architectureReport,compatibility:compatibilityReport,routingPlan:neutralRoutingPlan});
  }
  function ensureIntegrations(){ensureRoutingPlanScript();ensureCapabilityRegistryScript();ensureCiscoRoutingIntegrationScript();ensureMultivendorRoutingIntegrationScript();ensureFirewallEdgeIntegrationScript();ensureSwitchingIntegrationScript();ensureAccessSecurityIntegrationScript();ensureManagementIntegrationScript();ensureHaServicesIntegrationScript();}
  ...
  function bindBrowserUi(attempt){if(!root.document)return;ensureIntegrations();const gate=install(),state=root.NetWizardState;const button=root.document.getElementById('btnProductionGate'),output=root.document.getElementById('productionGateOut');if(!gate||!state||!button||!output){if((attempt||0)<50&&root.setTimeout)root.setTimeout(()=>bindBrowserUi((attempt||0)+1),120);return;}function runEnhanced(){const productionMode=root.NetWizardAudit&&root.NetWizardAudit.isProduction?root.NetWizardAudit.isProduction():false;const strict=!!root.document.getElementById('pgateStrict')?.checked;const report=gate.runProductionGate(state.getSnapshot(),{productionMode,strict});output.textContent=gate.summarizeGate(report,{limit:80,remediationPreview:root.document.getElementById('pgateShowGuide')?.checked?6:0});root.NetWizardLastProductionGateReport=report;return report;}button.onclick=runEnhanced;root.document.addEventListener('nw:project:changed',()=>{try{runEnhanced();}catch(_e){}});root.addEventListener&&root.addEventListener('nw:mode:changed',()=>{try{runEnhanced();}catch(_e){}});try{runEnhanced();}catch(_e){}}
  const api={version:'netwizard-production-gate-architecture-v10',install,enhanceReport,mergeIssues,ensureRoutingPlanScript,ensureCapabilityRegistryScript,ensureCiscoRoutingIntegrationScript,ensureMultivendorRoutingIntegrationScript,ensureFirewallEdgeIntegrationScript,ensureSwitchingIntegrationScript,ensureAccessSecurityIntegrationScript,ensureManagementIntegrationScript,ensureHaServicesIntegrationScript};root.NetWizardProductionGateArchitecture=api;if(typeof module!=='undefined'&&module.exports){install();module.exports=api;}if(root.document){if(root.document.readyState==='loading')root.document.addEventListener('DOMContentLoaded',()=>bindBrowserUi(0));else bindBrowserUi(0);}
