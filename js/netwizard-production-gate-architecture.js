...
(function initNetWizardProductionGateArchitecture(root){
  ...
  function ensureScript(src, selector, datasetKey){
    if(!root.document || root.document.querySelector(selector)) return;
    const script=root.document.createElement('script'); script.src=src; script.dataset[datasetKey]='1'; script.defer=false; root.document.head.appendChild(script);
  }
  function ensureRoutingPlanScript(){
    if(root.NetWizardRoutingPlan) return;
    ensureScript('./js/netwizard-routing-plan.js','script[data-netwizard-routing-plan]','netwizardRoutingPlan');
  }
  function ensureCiscoRoutingIntegrationScript(){
    if(root.NetWizardCiscoRoutingIntegration) return;
    ensureScript('./js/netwizard-cisco-routing-integration.js','script[data-netwizard-cisco-routing-integration]','netwizardCiscoRoutingIntegration');
  }
  function ensureMultivendorRoutingIntegrationScript(){
    if(root.NetWizardMultivendorRoutingIntegration) return;
    ensureScript('./js/netwizard-multivendor-routing-integration.js','script[data-netwizard-multivendor-routing-integration]','netwizardMultivendorRoutingIntegration');
  }
  function ensureFirewallEdgeIntegrationScript(){
    if(root.NetWizardFirewallEdgeIntegration) return;
    ensureScript('./js/netwizard-firewall-edge-integration.js','script[data-netwizard-firewall-edge-integration]','netwizardFirewallEdgeIntegration');
  }
  ...
  function enhanceReport(project, options, baseReport){
    const gate=baseGate(), architecture=validator();
    ...
    return Object.assign({},report,{ok:blocking.length===0,ready:status==='ready',canExport:blocking.length===0,status,issues,counts,architecture:architectureReport,routingPlan:neutralRoutingPlan});
  }
  ...
  function ensureIntegrations(){
    ...
    ensureFirewallEdgeIntegrationScript();
  }
  ...
  function install(){
    ...
    gate.runProductionGate=function(project,options){ return enhanceReport(project,options,originalRun(project,options)); };
  ...
  function bindBrowserUi(attempt){
    ...
    ensureIntegrations();
    const gate=install(), state=root.NetWizardState;
    const button=root.document.getElementById('btnProductionGate'), output=root.document.getElementById('productionGateOut');
    if(!gate||!state||!button||!output){ if((attempt||0)<50&&root.setTimeout) root.setTimeout(()=>bindBrowserUi((attempt||0)+1),120); return; }
    function runEnhanced(){
      ...
      root.NetWizardLastProductionGateReport=report; return report;
    }
  ...
  const api={version:'netwizard-production-gate-architecture-v5',install,enhanceReport,mergeIssues,ensureRoutingPlanScript,ensureCiscoRoutingIntegrationScript,ensureMultivendorRoutingIntegrationScript,ensureFirewallEdgeIntegrationScript};
