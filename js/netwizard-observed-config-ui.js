...
(function initNetWizardObservedConfigUi(root){
  ...
  function buildDevicePreview(project,deviceId,options){
    ...
    const generatedAt=iso(opts.generatedAt)||new Date().toISOString(),extension=clean(device.vendorOs,80)==='juniper_junos'?'set':'cfg',configPaths={[id]:`configs/${safeName(device.name,id)}.${extension}`},desiredConfigs={[id]:desired};
