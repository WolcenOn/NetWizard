...
function failoverGroupKey(c){return clean(c.siteRef||c.wanGroupRef||c.failoverGroupRef||c.locationRef||'global');}
...
return issues;}
function validateFailover(project,circuits){const issues=[];const groups=new Map();for(const c of circuits){const key=failoverGroupKey(c);if(!groups.has(key))groups.set(key,[]);groups.get(key).push(c);}for(const [key,list] of groups){const enabled=list.filter(c=>c.enabled!==false);if(enabled.length<2)continue;const providers=new Set(enabled.map(c=>clean(c.provider)).filter(Boolean));const paths=new Set(enabled.map(c=>clean(c.physicalPath||c.demarcLocation)).filter(Boolean));if(providers.size<2)issues.push({code:'NW-WAN-010',severity:'warning',blocking:false,category:'wan',siteRef:key,message:`WAN ${key}: múltiples circuitos comparten proveedor; la redundancia comercial es limitada.`});if(paths.size&&paths.size<2)issues.push({code:'NW-WAN-011',severity:'warning',blocking:false,category:'wan',siteRef:key,message:`WAN ${key}: los circuitos comparten demarcación o recorrido físico.`});const primary=enabled.find(c=>clean(c.role)==='primary')||enabled[0],backups=enabled.filter(c=>c!==primary),demand=requestedMbps(project,primary),backupCapacity=backups.reduce((s,c)=>s+(num(c.bandwidthUpMbps)||0),0);if(demand>backupCapacity)issues.push({code:'NW-WAN-012',severity:'warning',blocking:false,category:'wan',siteRef:key,message:`WAN ${key}: capacidad de respaldo ${backupCapacity} Mbps insuficiente para el pico ${demand} Mbps.`});}
return issues;}
function validateProject(project){const circuits=arr(project&&project.wanCircuits),issues=[];for(const c of circuits)issues.push(...validateCircuit(project||{},c));issues.push(...validateFailover(project||{},circuits));return{version:'netwizard-wan-circuits-v2',ok:!issues.some(i=>i.blocking),issues,counts:{blocking:issues.filter(i=>i.blocking).length,warnings:issues.filter(i=>i.severity==='warning').length},circuits:circuits.map(c=>({id:c.id,name:c.name,provider:c.provider,role:c.role,groupRef:failoverGroupKey(c),bandwidthDownMbps:c.bandwidthDownMbps,bandwidthUpMbps:c.bandwidthUpMbps,expectedPeakMbps:requestedMbps(project||{},c)}))};}
const api={version:'netwizard-wan-circuits-v2',validateCircuit,validateProject,requestedMbps,failoverGroupKey};root.NetWizardWanCircuits=api;if(typeof module!=='undefined'&&module.exports)module.exports=api;
