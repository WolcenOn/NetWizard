...
(function(){
  ...
  function icmpRuleAllows(a,b){
    ...
    const names=[a.name,b.name,va?.name,vb?.name,`VLAN ${va?.vlanId}`,`VLAN ${vb?.vlanId}`].map(x=>String(x||'').toLowerCase());
  ...
  function co(cls, text){ const d=document.createElement('div'); d.className='co '+cls; d.textContent=text; return d; }
  function render(){
